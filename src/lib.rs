...
use gguf::{ModelRunner, LoadedModel};
use std::sync::Arc;
...
impl auria_execution::ExecutionBackend for CpuBackendImpl {
    async fn execute_step(
        ...
        if self.model_runner.is_loaded().await {
            return self.model_runner.infer(
                &String::from_utf8_lossy(&input.data).to_string(),
                100,
                0.7,
                0.9,
            ).await.map(|tokens| ExecutionOutput {
                tokens,
                usage: UsageStats {
...
pub use gguf::{ModelRunner as GGUFModelRunner, LoadedModel, ModelConfig, ModelType};
